...
def test_secondary_model(tmpdir):
	with tmpdir.as_cwd():
		config = OptpressoConfig(model="nope", use_secondary_model=False, data_path="nothing.npy")
		...
		new_pred = 120.0
		assert secondary_model.predict([[new_pred]]) + new_pred == pytest.approx(new_pred)